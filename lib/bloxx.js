...
(function (global) {
	...
	function merge(target, source) {
		return Object.keys(source).reduce(function (target, key) {
			if ('type relationships'.indexOf(key) > -1) {
				return target;
			}
			var val = source[key];
	...
	function Model(spec) {
		Object.defineProperties(this, {
			...
			_allowedKeys: {
				value: assignArrayElements(spec.allowedKeys, [
					'type',
					'id',
					'relationships',
					'belongsTo'
			...
			_hasMany: {
				value: Object.freeze(spec.hasMany)
			},
			_belongsTo: {
				value: Object.freeze(spec.belongsTo)
			},
			_addHasMany: {
				value: []
			},
	...
	assign(Model.prototype, {
		fetch: function modelFetch(options) {
			options = options || {};
			var self = this;

			return this.store.get(this.type, this.id, options).then(function (attrs) {
				return attrs ? self.constructor.create(attrs) : null;
			});
		},

		load: function modelLoad(options) {
			var self = this;
			return this.fetch(options).then(function (res) {
				if (res) {
					return res;
				}
				var err = new Error(self.type + ' ' + self.id + ' not found');
				err.code = 'NOT_FOUND';
				return Promise.reject(err);
			});
		},

		fetchHasMany: function modelFetchHasMany(options) {
			options = options || {};
			var types = options.include === '*' ?
				this._hasMany : (options.include || this._hasMany);

			var self = this;
			var promises = types.map(function (type) {
				return self.store.getHasMany(self.id, type, options);
			});

			return flattenPromises(promises);
		},

		fetchBelongsTo: function modelFetchBelongsTo(options) {
			options = options || {};
			var types = options.include === '*' ?
				this._belongsTo : (options.include || this._belongsTo);

			var self = this;
			var promises = types.map(function (type) {
				return self.store.getBelongsTo(self.id, type, options);
			});

			return flattenPromises(promises);
		},

		loadHasMany: function modelLoadHasMany(options) {
			var self = this;
			this.getHasMany(options).then(function (items) {
				var promises = items.map(function (item) {
					return self.constructor.create(item).fetch(options);
				});

				return Promise.all(promises).then(function (entities) {
					self.relationships = reduceRelationships(entities, self.relationships);
					return self;
				});
			})
		},

		loadBelongsTo: function modelLoadBelongsTo(options) {
			var self = this;
			this.getBelongsTo(options).then(function (items) {
				var promises = items.map(function (item) {
					return self.registry.get(item.type).create(item).fetch(options);
				});

				return Promise.all(promises).then(function (entities) {
					self.belongsTo = reduceRelationships(entities, self.belongsTo);
					return self;
				});
			})
		},

		save: function modelSave(options) {
			options = options || {};
			var self = this;

			var promise = this.get(options).then(function (existing) {
				if (existing) {
					return existing.merge(self).put(options);
				}
				return this.put(options);
			});

			if (this._addHasMany.length) {
				promise = promise.then(function (entity) {
					var promises = self._addHasMany.map(function (rel) {
						return self.store.putRelationship(self.id, rel);
					});
					return Promise.all(promises).then(k(self));
				});
			}

			if (this._destroyHasMany.length) {
				promise = promise.then(function (entity) {
					var promises = self._destroyHasMany.map(function (rel) {
						return self.store.removeRelationship(self.id, rel.id);
					});
					return Promise.all(promises).then(k(self));
				});
			}
		...
		put: function modelPut(options) {
			return this.store.put(this.toJSON()).then(k(this));
		},

		destroy: function modelDestroy(options) {
			// Destroy all references
			// Then destroy the entity
		},

		merge: function modelMerge(attrs) {
			var target = this;
			Object.keys(attrs).reduce(function (target, key) {
				if ('type relationships'.indexOf(key) > -1) {
					return target;
				}
				var val = attrs[key];
	...
	function sortKeys(a, b) {
		...
		if (a === 'belongsTo') return -1;
		if (b === 'belongsTo') return +1;
		return 0;
	}

	function flattenPromises(promises) {
		return Promise.all(promises).then(function (results) {
			return results.reduce(function (items, collection) {
				return items.concat(collection);
			}, []);
		});
	}

	function reduceRelationships(source, target) {
		return source.reduce(function (target, entity) {
			if (!entity) {
				return target;
			}
			...
			target[entity.type].push(entity);
			return target;
		}, target);
	}
