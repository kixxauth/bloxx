...
(function (global, factory) {
	...
	BLOXX.ModelMixin = {
		...
		toJSON: function toJSON() {
			var self = this;
			return Object.keys(this).reduce(function (attrs, key) {
				var x = self[key];
				if (Array.isArray(x)) {
		...
		set: function (key, value) {
			...
			for (i; i < keys.length; i += 1) {
				...
				if (keys[i] === 'relationships') {
					throw new Error('Cannot set "relationships" on a Model instance');
				}
			}
			...
			var newModel = Object.create(Object.getPrototypeOf(this));
			newModel.initialize(attrs);
			return newModel;
		},
		...
		_createRecord: function createRecord(options) {
			...
			options.relationshipUpdates = rUpdates.length ? rUpdates : null;
		...
		_updateRecord: function updateRecord(options) {
			...
			if (this._relationshipUpdates.length) {
				this._relationshipUpdates.reduce(function (relationships, update) {
					var list = relationships[update.direction][update.type] || [];
					relationships[update.direction][update.type] = list;
