	...
	BLOXX.ModelMixin = {
		initializePrototype: function initializePrototype(proto) {
			...
			if (!proto.defaults || typeof proto.defaults !== 'object') {
				proto.defaults = Object.create(null);
			}
		...
		_createRecord: function createRecord(options) {
			...
			var privateAttrs = this.toJSON();
			attrs.id = this.generateId();
			privateAttrs.id = attrs.id;

			// Get the initial relationship updates.
			...
			// Concatenate all the relationship updates since this is a new model,
			// every relationship is a new relationship and needs to be created.
			...
			if (typeof this.beforeCreate === 'function') {
				promise = Promise.resolve(this.beforeCreate());
			}
		...
		_updateRecord: function updateRecord(options) {
			...
			var rUpdates = options.relationshipUpdates ?
				options.relationshipUpdates.slice() : null;
			...
			if (typeof this.beforeUpdate === 'function') {
				promise = Promise.resolve(this.beforeUpdate());
			}
			if (typeof this.beforeSave === 'function') {
				promise = promise.then(function () {
					return self.beforeSave();
		...
		_getRecord: function getRecord(options) {
			...
			if (typeof this.beforeFetch === 'function') {
				promise = Promise.resolve(this.beforeFetch());
			}
		...
		_removeRecord: function removeRecord(options) {
			...
			function removeRecord() {
				return self.removeRecord(attrs, options).then(function () {
					delete privateAttrs.relationships;
					newInstance.initialize(privateAttrs);
					return newInstance;
				});
			}
			...
			if (typeof this.beforeRemove === 'function') {
				promise = Promise.resolve(this.beforeRemove());
			}
